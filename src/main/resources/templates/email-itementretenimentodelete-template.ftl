<html xmlns="http://www.w3.org/1999/xhtml">
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>Java Mail</title>
</head>

<body>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td align="center" valign="top" bgcolor="#838383"
            style="background-color: #838383;"><br> <br>
            <table width="600" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td align="center" valign="top" bgcolor="#d3be6c"
                        style="background-color: #d3be6c; font-family: Arial, Helvetica, sans-serif; font-size: 13px; color: #000000; padding: 0px 15px 10px 15px;">

                        <div style="font-size: 48px; color:blue;">
                            <b>Java Mail</b>
                        </div>

                        <div style="font-size: 24px; color: #555100;">
                            <br> Sending Email using Spring Boot with <b>FreeMarker
                            template !!!</b> <br>
                        </div>
                        <div>
                            <br>"Sharing the knowledge is biggest learning" <br> <br>
                            <br> <br> <b>${nome} foi removido da nossa lista de filmes/séries !</b>
                            <br> <br> <b>Nós da DBCMovies trabalhamos para manter nossos conteúdos sempre atualizados !</b>
                            <br> <br> <b>Obrigado por sua compreensão !</b>
                            <br> <br> <b>Qualquer dúvida é só contatar o suporte pelo e-mail ${email}</b>
                            <br> <br> <b></b>
                            <br> <br> <b></b>
                            <br> <br> <b>Att,</b>
                            <br> <br> <b>Sistema.</b>

                            <br>
                        </div>
                    </td>
                </tr>
            </table> <br> <br></td>
    </tr>
</table>
</body>
</html>